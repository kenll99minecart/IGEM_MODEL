...
opt=odeset('RelTol',1e-10,'NonNegative',1);
size(Amountnull);
for (i=1:MaxS)
[ttnew,yy] = ode15s(@f, [0, Tfinal], EnvZi,opt);
...
%             EnvZP=0;
%         end
        dEnvZdt = ((a.*sodium(iter))./(sodium(iter)+b)).^c.*-EnvZ.*Phos+kad.*EnvZP;
        dEnvZPdt =((a.*sodium(iter))./(sodium(iter)+b)).^c.*EnvZ.*Phos-kad.*EnvZP;
        %dPhosdt = ((a.*sodium(iter))./(sodium(iter)+b)).^c.*-EnvZ.*Phos+kad.*EnvZP;
